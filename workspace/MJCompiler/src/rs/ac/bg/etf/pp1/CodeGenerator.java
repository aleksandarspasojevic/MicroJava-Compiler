...
//		log.info(Assignment.getDesignator().obj.getName());
		...
		Struct typeStruct = Assignment.getDesignator().obj.getType().getElemType().getElemType();
		Obj arrayAdr = Tab.insert(Obj.Var, "array", Tab.intType);
		...
		Code.store(column);  //column is on the top of the stack
		Code.put(Code.dup);
		Code.store(row);
